...
void UKF::Prediction(double delta_t)
{
    ...
    //Once sigma points are generated reset x_ and P_
...
/****************************************************************************/
void UKF::GenerateAugmentedSigmaPoints(MatrixXd& augmentedSigmaPoints)
{
    ...
    {
    	augmentedSigmaPoints.col(i+1) = x_aug + sqrt(lambda_ + n_aug_) * L.col(i);
    	augmentedSigmaPoints.col(i + 1 + n_aug_) = x_aug - sqrt(lambda_ + n_aug_) * L.col(i);
    }
}

void UKF::PredictSigmaPoints(const MatrixXd &sigmaPoints, double delta_t)
{
	  ...
	  for (int i = 0; i < (2* n_aug_)+ 1; i++)
	  {
			...
			double nu_a = 	sigmaPoints(5,i);
			double nu_yawdd = sigmaPoints(6,i);
...
/****************************************************************************/
void UKF::UpdateRadarMeasurement(MeasurementPackage meas_package)
{
